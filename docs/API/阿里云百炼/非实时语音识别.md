...
非实时语音识别支持在转写结果中输出时间戳，便于字幕生成、关键词高亮、音视频剪辑等场景。Fun-ASR、Qwen3-ASR-Flash-Filetrans、Paraformer 三个异步转写模型均支持，但各模型的时间戳默认行为和控制方式不同：

-   **Fun-ASR**：时间戳功能固定开启，不可关闭。
...
-   **Paraformer**：时间戳功能默认关闭，可通过请求参数 `timestamp_alignment_enabled` 设为 `true` 开启。
