...
**API 参考：**[定制热词 API 参考](https://help.aliyun.com/zh/model-studio/custom-hot-words/)
...
**使用场景：**通过 `input.messages` 传入上下文信息，可显著提升专有词汇（人名、地名、产品术语等）的转写准确率。适用于以下场景：

-   **词表增强：**识别单条音频时，通过 `user`（`input_text`）传入领域相关的词表或术语，帮助模型准确识别专业词汇。
...
-   **上下文机制：**上下文通过词表匹配方式生效，`text` 字段中需包含音频里待识别的原词（如"Kubernetes"、"Bulge Bracket"）。仅传入语义相关但不包含原词的描述不会生效。
